...
export function renderStatusLine(state: ZergState, options: RenderOptions = {}): string {
  ...
  return fit(`zerg v0.2.0 command surface | agents ${agents.length} (${running} running) | tasks ${tasks.length} | blocked ${blocked} | mode ${state.mode.automation}`, options.width);
}
...
export function renderHelp(state: ZergState, options: RenderOptions = {}): string {
  return [
    'pi-zerg-swarm v0.2.0 command-surface scaffold',
